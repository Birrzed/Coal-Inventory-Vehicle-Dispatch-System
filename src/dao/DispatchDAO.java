...
public class DispatchDAO {
    ...
    public List<Dispatch> getDispatchesByRole(String role, int userId) {
        ...
        if (role.equals("Admin")) {
            sql = "SELECT d.*, u1.username as transporter_name, u2.username as destination_name " +
                    "FROM dispatch d " +
                    "LEFT JOIN users u1 ON d.transporter_id = u1.id " +
                    "LEFT JOIN users u2 ON d.destination_id = u2.id";
        } else {
            switch (role) {
                case "Seller":
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                String tName = role.equals("Admin") ? rs.getString("transporter_name") : null;
                String dName = role.equals("Admin") ? rs.getString("destination_name") : null;
    ...
    public void deleteDispatch(int dispatchId) {
        String sql = "DELETE FROM dispatch WHERE id = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
